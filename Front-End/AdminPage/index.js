...
newUser.addEventListener('click',async (e)=>{
    ...
    try{
        const result = await axios.get(`http://54.210.91.135:3000/group/get-add-newUser?phone=${phone}&group=${group_name}`,{
            headers:{
                authenticate:token
            }
        });
...
removeUser.addEventListener('click',async (e)=>{
    ...
    try{
        const result = await axios.get(`http://54.210.91.135:3000/group/get-delete-User?phone=${phone}&group=${group_name}`,{
            headers:{
                authenticate:token
            }
        });
...
newAdmin.addEventListener('click',async (e)=>{
    ...
    try{
        const result = await axios.get(`http://54.210.91.135:3000/group/get-change-admin?phone=${phone}&group=${group_name}`,{
            headers:{
                authenticate:token
            }
        });
